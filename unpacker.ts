 ...
 */
async function instantiateFlutter({resPath, destPath, bundle}: InstantiatorContext) {
  fs.rmSync(destPath, {recursive: true, force: true});
  // run flutter create in a subprocess
  await $(`flutter create "${destPath}"`);
  // string replace paths in dev.nix
  const devnixPath = path.resolve(destPath, '.idx/dev.nix');
  // fs.chmodSync(devnixPath, 0o644);
  let devnix = fs.readFileSync(path.resolve(resPath, 'flutter/dev.nix'), { encoding: 'utf8' });
...
async function instantiateDefault({resPath, destPath, bundle}: InstantiatorContext) {
  ...
  const devnixPath = path.resolve(destPath, '.idx/dev.nix');
  fs.cpSync(path.resolve(resPath, 'default/dev.nix'), devnixPath);
  fs.chmodSync(devnixPath, 0o644);
  await unpackFiles(destPath, bundle);
}
